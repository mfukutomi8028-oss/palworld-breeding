...
- パル選択候補を30件制限から全件表示に変更
- 左側の親パル・結果パル絞り込みも画像付き検索UIに変更
- 詳細欄の「＋候補を追加」を削除し、閲覧専用であることが自然に分かる表現へ変更
- メモ欄は未入力時に「編集ボタンから入力できます」と表示
- 確認チェックリストのレイアウト崩れを修正
- 登録・詳細から「ゲーム版」項目を削除
- 削除・編集ボタンを詳細欄の上部に固定配置
- 一覧上部の検索欄を広く、更新日時プルダウンを適正幅に調整
- ササゾーなどの画像が取得できない場合にPalDB画像へフォールバックする処理を追加
- Palworld Labキャッシュをv4へ更新し、古い不完全なキャッシュを使い回しにくく変更
